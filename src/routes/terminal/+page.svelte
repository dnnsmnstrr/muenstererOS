<script lang="ts" module>
	type Line = { value: string; type?: 'input' | 'output' | 'ascii' };
...
<script lang="ts">
	...
	// Load command history and ASCII art from static on mount
	onMount(() => {
		...
		},
		{
			...
			callback: async ({ args, lines }) => {
				if (args.length === 0) {
					...
					// fetch ASCII art from static folder
					const skullText = await (await fetch('/ascii/skull.txt')).text();
					const skullLines = skullText.split(/\r?\n/);
					skullLines.forEach((l) => lines.push({ value: l, type: 'ascii' }));
					unlockAchievement('haxor');
				} else if (args[0] === 'image.png') {
					const duckText = await (await fetch('/ascii/duck.txt')).text();
					const duckLines = duckText.split(/\r?\n/);
					duckLines.forEach((l) => lines.push({ value: l, type: 'ascii' }));
				} else {
			...
			callback: async ({ args, lines }) => {
				if (commandHistory.length === 0) {
					...
					if (isNaN(index) || index < 0 || index >= commandHistory.length) {
						...
						lines.push({
							value: `Command at index ${index + 1}: ${commandHistory[index]}`,
							type: 'output'
						});
					}
				} else {
		...
		},
		{
			...
			callback: async ({ args, lines }) => {
				if (args.length === 0) {
					...
					if (!response.ok) {
						lines.push({
							value: `Error fetching ${args[0]}: ${response.statusText}`,
							type: 'output'
						});
						return;
					}
	...
	function handleCommandCompletion(command: string, query: string): string[] {
		switch (command) {
			case 'goto':
				return pages
					.map((page) => page.path || page.title.toLocaleLowerCase())
					.filter((page) => page.startsWith(query.toLowerCase()));
			case 'cd':
			...
			case 'curl':
				return endpoints
					...
					.filter((url) => url.startsWith(query || ''));
			case 'help':
				return commands
				...
				{#each lines as line}
					<span>
						...
						{:else if line.type === 'ascii'}
							<pre class="m-0 whitespace-pre">{line.value}</pre>
						{:else}
							{line.value}
						{/if}
					</span>
